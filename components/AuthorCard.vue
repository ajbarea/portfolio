  ...
  <div class="md:fixed">
    <div class="md:block flex justify-center items-center">
      <a href="https://ajbarea.github.io/" target="_blank">
        <div
          class="relative overflow-hidden rounded-full shadow-xl md:h-60 md:w-60 h-40 w-40 flex-shrink-0"
        >
          <nuxt-img
    ...
    <div class="mx-7 hidden md:block">
      ...
      <div class="my-2 text-gray-600 flex dark:text-blue-200">
        ...
        <a :href="siteMetadata.github" target="_blank">
          {{ `GitHub @${siteMetadata.githubUser}` }}</a
        >
      </div>
      <div class="my-2 text-gray-600 flex dark:text-blue-200">
        <LinkedIn />
        <a :href="siteMetadata.linkedin" target="_blank">
          {{ `LinkedIn @${siteMetadata.linkedinUser}` }}</a
        >
      </div>
      <div class="my-2 text-gray-600 flex dark:text-blue-200">
        <YouTube />
        <a :href="siteMetadata.youtube" target="_blank">
          {{ ` YouTube @ajbarea` }}</a
        >
      </div>
    </div>
...
export default {
  ...
  methods: {
    handleImageError(e) {
      ...
      this.imgError = true
      e.target.src =
        'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="100" height="100" viewBox="0 0 100 100"><rect width="100%" height="100%" fill="%23f0f0f0"/><text x="50%" y="50%" font-family="sans-serif" font-size="20" text-anchor="middle" dominant-baseline="middle" fill="%23999">AJ</text></svg>'
    }
  }
}
</script>
